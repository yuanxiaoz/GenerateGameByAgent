...
- **联机会话日志更可靠**：新增“联机日志队列 + 自动补发”机制，网络波动/后端短暂不可用时不再直接丢失；并在登录/启动时自动补发。
- **日志回显更及时**：会话日志页监听日志上报事件，当前会话可自动刷新。

### 🧹 代码结构

- **服务层拆分**：将 `request` 的“HTTP 客户端能力”和业务 API 拆分为 `httpClient` 与 `services/api/*`，同时保留 `src/services/request.ts` 作为兼容出口，降低耦合与单文件复杂度。
